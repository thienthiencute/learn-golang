...
(function () {
	...
	function pluginData() {
		...
		function initializeChoices(container = document) {
			var choicesExamples = container.querySelectorAll("[data-choices]");

			Array.from(choicesExamples).forEach(function (item) {
				// Skip if already initialized
				if (item.classList.contains('choices__input') || item._choicesInstance) {
					return;
				}
		...
		document.addEventListener('reinitializeChoices', function (event) {
			if (event.detail && event.detail.container) {
				initializeChoices(event.detail.container);
			}
		});
	...
	function twoColumnMenuGenerate() {
		...
		if (defaultValues && (isTwoColumn == "twocolumn" || defaultValues["data-layout"] == "twocolumn")) {
			...
			ul.innerHTML = '<a href="#" class="logo"><img src="/static/themes/images/logo-sm.png" alt="" height="22"></a>';
	...
	function windowResizeHover() {
		...
		if (windowSize < 1025 && windowSize > 767) {
			...
			if (sessionStorage.getItem("data-layout") == "twocolumn") {
				...
				// Sử dụng SidebarNavigation thay vì initTwoColumnActiveMenu
				if (window.sidebarNavigationInstance) {
					window.sidebarNavigationInstance.updateActiveMenu();
				} else {
	...
	function windowLoadContent() {
		...
		window.addEventListener("load", function () {
			// Import và sử dụng SidebarNavigation class
			if (window.SidebarNavigation) {
				// SidebarNavigation sẽ tự động xử lý cả vertical và two-column layout
				if (!window.sidebarNavigationInstance) {
					window.sidebarNavigationInstance = new window.SidebarNavigation();
				}
			}
			// Removed the else block that called initTwoColumnActiveMenu() or initActiveMenu()
			isLoadBodyElement();
	...
	function initTwoColumnActiveMenu() {
		...
		// Xóa tất cả active states trước khi set mới
		clearTwoColumnActiveStates();

		if (currentPath) {
			// Tìm kiếm exact match trước
			var exactMatch = findExactMatch(currentPath);
			if (exactMatch) {
				setTwoColumnActiveState(exactMatch);
				return;
			}

			// Nếu không có exact match, tìm kiếm partial match
			var partialMatch = findPartialMatch(currentPath);
			if (partialMatch) {
				setTwoColumnActiveState(partialMatch);
				return;
			}

			// Fallback: nếu là trang chủ
			if (currentPath === "/" || currentPath === "") {
				var homeLink = document.getElementById("navbar-nav").querySelector('[href=""], [href="/"]');
				if (homeLink) {
					setTwoColumnActiveState(homeLink);
				}
			}
		}
	}

	// Hàm xóa tất cả active states cho two-column layout
	function clearTwoColumnActiveStates() {
		// Xóa active class từ two-column menu
		var twoColumnMenu = document.getElementById("two-column-menu");
		if (twoColumnMenu) {
			var allTwoColumnLinks = twoColumnMenu.querySelectorAll("a");
			allTwoColumnLinks.forEach(function (link) {
				link.classList.remove("active");
			});
		}

		// Xóa twocolumn-item-show class
		var allTwoColumnItems = document.querySelectorAll(".twocolumn-item-show");
		allTwoColumnItems.forEach(function (item) {
			item.classList.remove("twocolumn-item-show");
		});

		// Gọi hàm clear thông thường
		clearActiveStates();
	}

	// Set active state cho two-column layout
	function setTwoColumnActiveState(activeLink) {
		if (!activeLink) return;

		// Set active state thông thường trước
		setActiveState(activeLink);

		// Xử lý two-column specific logic
		if (document.body.classList.contains("twocolumn-panel")) {
			var twoColumnMenu = document.getElementById("two-column-menu");
			if (twoColumnMenu) {
				var href = activeLink.getAttribute("href");
				var twoColumnLink = twoColumnMenu.querySelector('[href="' + href + '"]');
				if (twoColumnLink) {
		...
		// Xử lý parent collapse menus cho two-column
		var parentCollapseDiv = activeLink.closest(".collapse.menu-dropdown");
		if (parentCollapseDiv) {
			var parentElement = parentCollapseDiv.parentElement;

			// Kiểm tra nếu có nested collapse
			var grandParentCollapse = parentElement.closest(".collapse.menu-dropdown");
			if (grandParentCollapse) {
				// Nested menu case
				grandParentCollapse.parentElement.classList.add("twocolumn-item-show");

				// Set active cho two-column menu icon
				var menuId = grandParentCollapse.getAttribute("id");
				var twoColumnMenu = document.getElementById("two-column-menu");
				if (twoColumnMenu && menuId) {
					var twoColumnToggle = twoColumnMenu.querySelector('[href="#' + menuId + '"]');
					if (twoColumnToggle) {
						twoColumnToggle.classList.add("active");
					}
				}

				// Xử lý sub-nested menu
				var greatGrandParentCollapse = grandParentCollapse.parentElement.closest(".collapse.menu-dropdown");
				if (greatGrandParentCollapse) {
					var subMenuId = greatGrandParentCollapse.getAttribute("id");
					greatGrandParentCollapse.parentElement.classList.add("twocolumn-item-show");
					grandParentCollapse.parentElement.classList.remove("twocolumn-item-show");

					if (twoColumnMenu && subMenuId) {
						var subTwoColumnToggle = twoColumnMenu.querySelector('[href="#' + subMenuId + '"]');
						if (subTwoColumnToggle) {
							subTwoColumnToggle.classList.add("active");
						}
					}
				}
			} else {
			...
			// Nếu không tìm thấy active link, show two-column panel
			document.body.classList.add("twocolumn-panel");
		}
	}
	...
	function initActiveMenu() {
		...
		if (currentPath) {
			...
			if (currentPath === "/" || currentPath === "") {
				var homeLink = document.getElementById("navbar-nav").querySelector('[href=""], [href="/"]');
				if (homeLink) {
					setActiveState(homeLink);
				}
			}
		}
	}

	// Hàm xóa tất cả active states
	function clearActiveStates() {
		...
		var allCollapses = document.querySelectorAll("#navbar-nav .collapse");
		allCollapses.forEach(function (collapse) {
			collapse.classList.remove("show");
		});

		// Reset aria-expanded
		var allToggleLinks = document.querySelectorAll("#navbar-nav [data-bs-toggle='collapse']");
	...
	function findExactMatch(currentPath) {
		var allLinks = document.querySelectorAll("#navbar-nav a[href]");
		for (var i = 0; i < allLinks.length; i++) {
			var link = allLinks[i];
			var href = normalizePath(link.getAttribute("href")); // Normalize href as well
			if (href === currentPath) {
				return link;
			}
		}
	...
	// Tìm kiếm partial match (cho trường hợp URL có parameters hoặc sub-paths)
	function findPartialMatch(currentPath) {
		var allLinks = document.querySelectorAll("#navbar-nav a[href]");
		var bestMatch = null;
		var longestMatch = 0;
		...
		for (var i = 0; i < allLinks.length; i++) {
			...
			// Ensure href is not empty and not just "/"
			if (href && href !== "/") {
				// Check if currentPath starts with href and the next character in currentPath is either end of string or a '/'
				if (currentPath.startsWith(href) && (currentPath.length === href.length || currentPath.charAt(href.length) === '/')) {
	...
	function setActiveState(activeLink) {
		...
		if (parentCollapseDiv) {
			...
			var toggleButton = parentCollapseDiv.parentElement.children[0];
			if (toggleButton) {
				toggleButton.classList.add("active");
				toggleButton.setAttribute("aria-expanded", "true");
			}

			// Xử lý nested collapse (level 2)
			var grandParentCollapse = parentCollapseDiv.parentElement.closest(".collapse.menu-dropdown");
			if (grandParentCollapse) {
				grandParentCollapse.classList.add("show");
				var grandParentToggle = grandParentCollapse.previousElementSibling;
				if (grandParentToggle) {
					grandParentToggle.classList.add("active");
					grandParentToggle.setAttribute("aria-expanded", "true");
				}

				// Xử lý nested collapse (level 3)
				var greatGrandParentCollapse = grandParentCollapse.parentElement.closest(".collapse.menu-dropdown");
				if (greatGrandParentCollapse) {
					greatGrandParentCollapse.classList.add("show");
					var greatGrandParentToggle = greatGrandParentCollapse.previousElementSibling;
					if (greatGrandParentToggle) {
						greatGrandParentToggle.classList.add("active");
						greatGrandParentToggle.setAttribute("aria-expanded", "true");
					}
				}
			}
		}
					...
					if (elem.querySelectorAll(".notification-item").length > 0) {
						...
						if (!emptyNotificationElem) {
							...
							<div class="w-25 w-sm-50 pt-3 mx-auto">\
								<img src="/static/themes/images/svg/bell.svg" class="img-fluid" alt="user-pic">\
							</div>\
	...
	function hideShowLayoutOptions(dataLayout) {
		if (dataLayout == "vertical") {
			...
			// Sử dụng SidebarNavigation thay vì initActiveMenu
			if (window.sidebarNavigationInstance) {
				window.sidebarNavigationInstance.updateActiveMenu();
			} else {
				initActiveMenu();
	...
	function addEventListenerOnSmHoverMenu() {
		var storedState = localStorage.getItem("sidebarSize");
		if (storedState) {
			document.documentElement.setAttribute("data-sidebar-size", storedState);
		}
		document.getElementById("vertical-hover").addEventListener("click", function () {
			if (document.documentElement.getAttribute("data-sidebar-size") === "sm-hover") {
				document.documentElement.setAttribute("data-sidebar-size", "sm-hover-active");
				localStorage.setItem("sidebarSize", "sm-hover-active");
			} else if (document.documentElement.getAttribute("data-sidebar-size") === "sm-hover-active") {
				document.documentElement.setAttribute("data-sidebar-size", "sm-hover");
				localStorage.setItem("sidebarSize", "sm-hover");
			} else {
				document.documentElement.setAttribute("data-sidebar-size", "sm-hover");
				localStorage.setItem("sidebarSize", "sm-hover");
			}
